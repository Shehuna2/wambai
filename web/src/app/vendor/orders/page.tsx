...
export default function VendorOrdersPage() {
  ...
  return (
    <div className="space-y-3">
      <h1 className="text-2xl font-extrabold text-slate-900">Vendor orders</h1>
      {error && <p className="text-red-600">{error}</p>}
      {loading ? <p>Loading...</p> : (
        <div className="wb-shell overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-green-50"><tr><th className="p-2 text-left">Order ID</th><th>Shop</th><th>Subtotal</th><th>Status</th><th>Created</th><th></th></tr></thead>
            <tbody>
              {orders.map((o) => (
